...
<!-- 这个代码片段实现了一个简单的结算页面，包括地址选择、商品信息展示、配送时间选择、支付方式选择、金额明细和提交订单功能。
逻辑如下：
导入所需的模块和库，包括结算信息和创建订单的API、Vue Router、Vue的响应式引用和生命周期钩子，以及购物车存储库。
创建购物车存储实例和路由实例。
定义响应式引用变量 checkInfo 和 curAddress，分别用于存储订单信息和当前选择的地址。
创建异步函数 getCheckInfo，用于获取结算信息。在函数内部，调用结算信息的API，并将返回的结果赋值给 checkInfo 变量。同时，从地址列表中筛选出 isDefault === 0 的地址，并将其赋值给 curAddress 变量。
使用生命周期钩子 onMounted 在组件挂载后调用 getCheckInfo 函数，以获取结算信息。
定义响应式引用变量 showDialog，用于控制切换地址弹框的显示与隐藏。
定义响应式引用变量 activeAddress，用于存储当前选中的地址。
定义函数 switchAddress，接受一个参数 item，将其赋值给 activeAddress 变量，实现切换当前选中的地址功能。
定义函数 confirm，用于确认选择的地址为当前地址。在函数内部，将 activeAddress 的值赋给 curAddress，关闭切换地址弹框，同时重置 activeAddress 的值为空对象，以准备下次选择。
定义异步函数 createOrder，用于创建订单。在函数内部，调用创建订单的API，并传递所需的参数，包括配送时间类型、支付类型、支付渠道、买家留言、商品信息和地址ID。获取返回的订单ID，并通过路由跳转到支付页面，并将订单ID作为查询参数传递。最后，调用购物车存储库的方法更新购物车。
在HTML模板中，使用Vue指令和绑定语法来渲染页面的各个部分，包括收货地址、商品信息、配送时间、支付方式和金额明细。通过监听按钮的点击事件，触发相应的方法。
使用 el-dialog 组件实现切换地址的弹框功能。通过监听弹框的确认按钮点击事件，触发 confirm 方法。
以上就是这个代码片段的基本实现逻辑。请注意，这只是代码片段的一部分，可能会依赖其他代码和组件来完成整个结算页面的功能。 -->
